...
class Post extends Base_Admin_Controller {
    ...
    public function add() {
        ...
        if ( isset($_POST['add']) ) {
            ...
            $category_id = (int)$this->params['pid'];
            if($category_id > 0) {
                $category = $this->category_admin_model->get_by_id($category_id);
                $langmap_id = (int)$category->langmap_id;

                $category_list = $this->category_admin_model->get_post_by_langmap_id($langmap_id);

                foreach($this->languages as $l) {
                    $this->data['category_list'][$l] = get_list_by_language_id($l, $category_list, true);
                }  
            }else {
                 foreach($this->languages as $l) {
                    $this->data['category_list'][$l] = array();
                }
            }
    ...
    public function edit() {
        ...
        if ( isset($_POST['update']) ) {
            ...
            $rs = $this->post_admin_model->get_post_by_langmap_id($langmap_id);
            foreach($this->languages as $l) {
    ...
    private function sync_order() {
        ...
        if( $category_id > 0) {
            ...
            if( !empty($category) ) {
                $langmap_id = (int)$category->langmap_id;
                $list_by_langmap = $this->category_admin_model->get_by_langmap_id($langmap_id);
                foreach($list_by_langmap as $l) {
                    $data = array(
                        'category_id' => $l['category_id'],
                        'post_type' => 'post',
                        'language_id' => $l['language_id'],
                        'post_module' => $this->module_code()
                    );
                    $this->post_admin_model->sync($data);
                }
            }
