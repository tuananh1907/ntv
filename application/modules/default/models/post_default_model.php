...
class Post_Default_Model extends Post_Model
{
    ...
    public function get_posts($post_module, $language_id, $meta = array()) {
    ...
    public function get_post_by_category($post_module, $language_id, $category_id, $meta = array()) {
        $this->db->select('post.*, alias.alias_name');
        $this->db->from($this->get_table());
        $this->db->join('alias', 'alias.fid = post.post_id and alias.alias_module = post.post_module', 'left');
    ...
    public function get_pages($language_id = '')
    {
        $this->db->select('*');
        $this->db->from($this->get_table());
        $this->db->where(array('post_type' => 'page', 'post_status' => 1));
    ...
    public function get_post_by_pagination($post_module, $language_id, $offset, $limit, $category_id = 0)
    {
        $this->db->select('post.*, alias.alias_name');
        $this->db->from($this->get_table());
        $this->db->join('alias', 'alias.fid = post.post_id and alias.alias_module = post.post_module');
